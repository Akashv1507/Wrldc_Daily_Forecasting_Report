...
class IntradayForecastVsActualFetchRepository():
    ...
    def toContextDict(self, dataDf:pd.core.frame.DataFrame):
        ...
        for col in colName:
            ...
            singleColObj = {'colName': col, 'maxFor': filteredDf['MAX_FORECAST'][0].round(0).astype(int), 'maxDem':filteredDf['MAX_DEMAND'][0].round(0).astype(int),
    ...
    def fetchForVsActualData(self, startTime: dt.datetime, endTime: dt.datetime):
        """ fetch  Intraday Forecast Vs Actual dataand return context
        Args:
        ...
        try:
            ...
            cur.close()
            connection.close()
        constextData = self.toContextDict(dataDf)
        return constextData
